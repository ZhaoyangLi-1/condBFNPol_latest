"""Training Workspace for Diffusion Policy on Robomimic tasks.

This workspace handles training of the DiffusionUnetHybridImagePolicy on Robomimic
manipulation tasks (Lift, Can, Square, etc.). It follows the same structure as the
original diffusion_policy repo for fair comparison with BFN.
"""
...
__all__ = ["TrainDiffusionRobomimicWorkspace"]

# Register OmegaConf resolver for eval
OmegaConf.register_new_resolver("eval", eval, replace=True)


class TrainDiffusionRobomimicWorkspace(BaseWorkspace):
    """Workspace for training Diffusion Policy on Robomimic tasks.
...
def _main(cfg):
    ...
    workspace = TrainDiffusionRobomimicWorkspace(cfg)
    workspace.run()
...
if HAS_HYDRA:
    @hydra.main(
        ...
        config_name="train_diffusion_robomimic_image"
    )
    def main(cfg):
        _main(cfg)
else:
