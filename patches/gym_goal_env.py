...
class GoalEnv(gym.Env):
    ...
    def __draw_render_states(self):
        """
        Draw the trail of recent positions: latest state in full detail, past
        positions as translucent dots fading by age.
        """
        if not self.render_states:
            return
        TRAIL_WINDOW = 100
        states = self.render_states[-TRAIL_WINDOW:]

        # Latest state drawn in full (clears window + draws field + agents)
        self.__draw_internal_state(states[-1])

        # Overlay fading trail dots for older positions
        if len(states) > 1:
            past = states[:-1]
            n = len(past)
            trail_surf = pygame.Surface(self.window.get_size(), pygame.SRCALPHA)
            for i, rs in enumerate(past):
                alpha = int(40 + (200 - 40) * (i / max(1, n - 1))) if n > 1 else 100
                ppos = tuple(self.__upscale(rs[0]).astype(int))
                gpos = tuple(self.__upscale(rs[2]).astype(int))
                bpos = tuple(self.__upscale(rs[4]).astype(int))
                pygame.draw.circle(trail_surf, (255, 255, 255, alpha), ppos, 6)
                pygame.draw.circle(trail_surf, (255, 0, 0, alpha), gpos, 6)
                pygame.draw.circle(trail_surf, (0, 0, 0, alpha), bpos, 3)
            self.window.blit(trail_surf, (0, 0))
            pygame.display.update()

        self.render_states = states  # keep rolling window
    ...
    def render(self, mode='human', close=False):
        ...
        if mode == 'rgb_array':
            import numpy as np
            pygame.display.flip()
            arr = pygame.surfarray.array3d(self.window)
