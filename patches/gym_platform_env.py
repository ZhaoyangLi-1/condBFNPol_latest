...
class PlatformEnv(gym.Env):
    ...
    def _draw_render_states(self, mode="human"):
        """
        Draw the trail of recent player positions on a single surface.
        Player trail fades (older = more transparent); enemies drawn only at
        the latest state to avoid visual clutter.
        """
        self._draw_background()
        TRAIL_WINDOW = 100  # effectively keep all past positions in an episode
        ...
        for i, rs in enumerate(states):
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    pygame.quit()
                    sys.exit()
            # Linear alpha fade: oldest ~ 50, newest = 255 (player AND enemies)
            if n > 1:
                alpha = int(50 + (255 - 50) * (i / (n - 1)))
            else:
                alpha = 255
            psprite = self.player_sprite.copy(); psprite.set_alpha(alpha)
            esprite = self.enemy_sprite.copy(); esprite.set_alpha(alpha)
            self._draw_sprite(psprite, rs[0], self.player.size)
            self._draw_sprite(esprite, rs[1], self.enemy1.size)
            self._draw_sprite(esprite, rs[2], self.enemy2.size)
        self._draw_to_screen(255)
        # Keep only the rolling window for next render
