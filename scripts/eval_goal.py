"""Evaluate BFN, DDPM, DDIM, EDM, Consistency on Goal environment."""
...
print("Starting Goal eval...", flush=True)
...
NUM_DISCRETE = 3
ACTION_DIM_CONT = 5  # 3 one-hot + 2 padded params
...
def load_policy(checkpoint_path, policy_type, device="cuda"):
    ...
    if policy_type == "bfn":
        policy = BFNHybridLowdimPolicy(
            obs_dim=17, horizon=16, n_obs_steps=2, n_action_steps=8,
            num_discrete_actions=3, continuous_param_dim=2,
        ...
        policy = EDMLowdimPolicy(
            obs_dim=17, action_dim=5, horizon=16, n_obs_steps=2, n_action_steps=8,
...
def get_action_continuous(policy, obs_history):
    ...
    return {"k": k, "x_k": action[NUM_DISCRETE:NUM_DISCRETE + 2]}
...
def evaluate(policy, policy_type, n_episodes=50, device="cuda"):
    ...
    for ep in range(n_episodes):
        ...
        successes.append(float(total_reward >= 40))
        if (ep + 1) % 10 == 0:
            print(f"  Episode {ep+1}/{n_episodes}: reward={total_reward:.2f}", flush=True)
...
def main():
    ...
    for ptype, ckpt, label in eval_specs:
        ...
        r = results[ptype]
        print(f"\n{label}: {r['success_rate']:.1f}% / {r['mean_reward']:.2f} ± {r['std_reward']:.2f} / {r['mean_inference_ms']:.1f} ms")
    ...
    if len(results) >= 2:
        ...
        for label, ptype in [("BFN(20)", "bfn"), ("BFN-10", "bfn10"), ("DDPM(100)", "ddpm"),
            ...
            if ptype in results:
                r = results[ptype]
                print(f"  {label:<12} {r['success_rate']:>6.1f}%  {r['mean_reward']:>7.2f}  {r['mean_inference_ms']:>7.1f} ms")
