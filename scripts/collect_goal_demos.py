"""Collect demonstrations from trained DLPA expert on Goal environment."""
...
def collect(checkpoint_path: str, n_episodes: int = 500, output_path: str = None):
    args = make_args()
    print(f"Creating DLPA trainer for Goal...")
    ...
    if output_path is None:
        output_path = "data/goal/replay.zarr"
    ...
    for ep in range(n_episodes):
        ...
        while not terminal:
            ...
            # Pad to 2D
            padded = ap[:2] if len(ap) >= 2 else np.concatenate([ap, np.zeros(2 - len(ap))])
            episode_x.append(padded.astype(np.float32))
        ...
        # Goal: success when reward >= 40 (scored)
        if total_reward >= 40:
            success_count += 1

        if (ep + 1) % 25 == 0:
            recent = episode_returns[-25:]
            print(f"Episode {ep+1}/{n_episodes}: recent_mean={np.mean(recent):.2f}, "
                  f"success={sum(1 for r in recent if r >= 40)/len(recent)*100:.1f}%")
