"""Collect demonstrations from trained DLPA expert on Platform environment."""
...
def collect(checkpoint_path: str, n_episodes: int = 500, output_path: str = None):
    args = make_args()
    print(f"Creating DLPA trainer for Platform...")
    ...
    if output_path is None:
        output_path = "data/platform/replay.zarr"
    ...
    for ep in range(n_episodes):
        ...
        episode_k, episode_x, episode_rewards = [], [], []
        terminal = False
        t = 0
        total_reward = 0.
        ...
        # Platform: success when reward > 0.9 (almost reached end)
        if total_reward > 0.9:
            success_count += 1

        if (ep + 1) % 25 == 0:
            recent = episode_returns[-25:]
            print(f"Episode {ep+1}/{n_episodes}: recent_mean={np.mean(recent):.3f}, "
                  f"success={sum(1 for r in recent if r > 0.9)/len(recent)*100:.1f}%")
    ...
    print(f"\n=== Demo Collection Complete (Platform) ===")
    print(f"Episodes: {n_episodes}, Steps: {total_steps}")
    print(f"Mean reward: {np.mean(episode_returns):.3f} ± {np.std(episode_returns):.3f}")
...
def main():
    ...
    collect(args.checkpoint, args.n_episodes, args.output)
