"""Collect demonstrations from trained DLPA expert on Hard Goal environment.
...
def make_dlpa_args(checkpoint_path: str = None):
    class Args:
        env = "hard_goal-v0"
        seed = 0
...
def collect_demonstrations(checkpoint_path: str, n_episodes: int = 500,
    ...
    args = make_dlpa_args()
    print(f"Creating DLPA trainer for Hard Goal...")
    ...
    if output_path is None:
        output_path = "data/hard_goal/replay.zarr"
    ...
    for ep in range(n_episodes):
        ...
        while not terminal:
            ...
            if len(ap) >= 2:
                ...
                padded = np.concatenate([ap, np.zeros(2 - len(ap))])
        ...
        # Goal reward of 50 = success in HyAR Goal-v0 (kicked in)
        if total_reward >= 40:
            success_count += 1
        ...
        if verbose and (ep + 1) % 25 == 0:
            ...
            print(f"Episode {ep+1}/{n_episodes}: "
                  ...
                  f"recent_success={sum(1 for r in recent if r >= 40)/len(recent)*100:.1f}%, "
    ...
    print(f"\n{'='*60}")
    print(f"Demo Collection Complete (Hard Goal)")
    print(f"{'='*60}")
