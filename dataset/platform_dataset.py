"""Platform Dataset for BFN/Diffusion Policy training.

Platform-v0: 3 discrete actions, 1 continuous param each.
...
class PlatformReplayBuffer:
    def __init__(self, zarr_path: str):
        ...
        print(f"PlatformReplayBuffer: Episodes={self.n_episodes}, Steps={len(self)}")
...
class PlatformDataset(BaseLowdimDataset):
    def __init__(self, zarr_path: str, horizon: int = 16, pad_before: int = 1,
        ...
        self.replay_buffer = PlatformReplayBuffer(zarr_path)
        ...
        self.sampler = PlatformSequenceSampler(
        ...
        if action_mode == "hybrid":
            self.action_dim = 1 + self.max_param_dim  # [k, x] = 2
        else:
            self.action_dim = self.num_discrete + self.max_param_dim  # [one_hot, x] = 4

        print(f"PlatformDataset: episodes={self.replay_buffer.n_episodes}, "
              f"action_dim={self.action_dim}, action_mode={action_mode}")

    def get_validation_dataset(self):
        val_set = copy.copy(self)
        val_set.sampler = PlatformSequenceSampler(
