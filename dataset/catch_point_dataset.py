"""Catch Point Dataset for BFN/Diffusion Policy training.

Catch Point action space (2 discrete actions, 1 continuous param):
- act 0: MOVE (uses 1D continuous angle param)
- act 1: CATCH (no continuous param, padded to 0)

zarr format:
- data/state: [N, 6]
- data/action_k: [N] discrete index in [0, 1]
- data/action_x: [N] continuous angle param
...
class CatchPointReplayBuffer:
    def __init__(self, zarr_path: str):
        ...
        print(f"CatchPointReplayBuffer:")
...
class CatchPointSequenceSampler:
...
class CatchPointDataset(BaseLowdimDataset):
    """Catch Point dataset for BFN/Diffusion Policy training."""
    ...
    def __init__(
        ...
        self.replay_buffer = CatchPointReplayBuffer(zarr_path)
        ...
        self.sampler = CatchPointSequenceSampler(
        ...
        if action_mode == "hybrid":
            self.action_dim = 1 + self.max_param_dim  # [k, angle] = 2
        else:
            self.action_dim = self.num_discrete + self.max_param_dim  # [one_hot, angle] = 3

        print(f"CatchPointDataset initialized:")
    ...
    def get_validation_dataset(self):
        ...
        val_set.sampler = CatchPointSequenceSampler(
            replay_buffer=self.replay_buffer,
    ...
    def _process_episode(self, episode):
        ...
        cont_x = episode['action_x'].astype(np.float32)
        if cont_x.ndim == 1:
            cont_x = cont_x[:, None]

        if self.action_mode == "hybrid":
            action = np.concatenate([discrete_k[:, None].astype(np.float32), cont_x], axis=-1)
        else:
            one_hot = np.zeros((len(discrete_k), self.num_discrete), dtype=np.float32)
            one_hot[np.arange(len(discrete_k)), discrete_k] = 1.0
            action = np.concatenate([one_hot, cont_x], axis=-1)
        return state, action
    ...
    def __getitem__(self, idx: int):
        ...
        cont_x = sample['action_x'].astype(np.float32)
        if cont_x.ndim == 1:
            cont_x = cont_x[:, None]
...
class CatchPointHybridDataset(CatchPointDataset):
    def __init__(self, zarr_path: str, **kwargs):
        kwargs['action_mode'] = 'hybrid'
        super().__init__(zarr_path, **kwargs)


class CatchPointContinuousDataset(CatchPointDataset):
    def __init__(self, zarr_path: str, **kwargs):
