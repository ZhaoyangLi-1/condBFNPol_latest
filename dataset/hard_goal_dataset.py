"""Hard Goal Dataset for BFN/Diffusion Policy training.

Hard Goal action space (11 discrete actions, max 2 continuous params):
- act 0: KICK_TO (uses 2D continuous params)
- act 1-5: SHOOT_GOAL_LEFT (uses 1D continuous param)
- act 6-10: SHOOT_GOAL_RIGHT (uses 1D continuous param)

Padded to max param size = 2 for unified representation.

zarr format:
- data/state: [N, 17]
- data/action_k: [N] discrete index in [0, 10]
- data/action_x: [N, 2] padded continuous params
...
class HardGoalReplayBuffer:
    """Replay buffer for Hard Goal data."""

    def __init__(self, zarr_path: str):
        self.root = zarr.open(str(zarr_path), mode='r')
        self.data = self.root['data']
        self.num_discrete = 11
        self.max_param_dim = 2
...
class HardGoalSequenceSampler:
    """Sequence sampler for Hard Goal data."""
...
class HardGoalDataset(BaseLowdimDataset):
    """Hard Goal dataset for BFN/Diffusion Policy training."""
    ...
    def __init__(
        ...
        self.replay_buffer = HardGoalReplayBuffer(zarr_path)
        ...
        self.sampler = HardGoalSequenceSampler(
        ...
        if action_mode == "hybrid":
            ...
            self.action_dim = self.num_discrete + self.max_param_dim  # [one_hot, p1, p2] = 13

        print(f"HardGoalDataset initialized:")
    ...
    def get_validation_dataset(self):
        ...
        val_set.sampler = HardGoalSequenceSampler(
            replay_buffer=self.replay_buffer,
...
class HardGoalHybridDataset(HardGoalDataset):
    """Hybrid action mode: [k, p1, p2] (3D)."""
    def __init__(self, zarr_path: str, **kwargs):
        kwargs['action_mode'] = 'hybrid'
        super().__init__(zarr_path, **kwargs)


class HardGoalContinuousDataset(HardGoalDataset):
    """Continuous action mode: [one_hot(11), p1, p2] (13D)."""
    def __init__(self, zarr_path: str, **kwargs):
